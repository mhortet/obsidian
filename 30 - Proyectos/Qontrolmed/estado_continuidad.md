...
- Primer corte de `login/acceso` productivo confirmado en codigo el 2026-06-14:
  ...
  - validacion manual especifica del bootstrap completada el 2026-06-15 en
    ...
    - la restauracion inicial de `gestion_restore` vino desde una foto legacy
      y obligo a aplicar la migracion de roles antes de probar el caso final;
    - en escenario seguro con un unico usuario activo `superusuario`, sin
      `password_hash` y con `QONTROLMED_ALLOW_PASSWORDLESS_LOGIN=1`, la app ya
      pide contrasena inicial y repeticion;
    - tras fijar la contrasena inicial, la sesion entra correctamente y
      permite abrir `Gestion -> Usuarios` y `Gestion -> Roles`;
    - durante esa validacion se detecto y corrigio un bug en
      `src/Compras/services/usuarios_service.py` al refrescar la cuenta tras el
      bootstrap:
      - el flujo reutilizaba `idUsuario` como entero y lanzaba
...
`Ordenar configuracion local y cerrar documentalmente login/acceso`

Objetivo inmediato:

- dar por estable el primer corte de `login/acceso` ya validado:
...
- foto tecnica confirmada antes de programar:
  ...
  - la deuda abierta ya no es probarlo, sino dejarlo mejor documentado y
    simplificar la configuracion del entorno para futuras validaciones.
...
1. devolver el entorno temporal a su estado habitual si al cerrar la sesion aun
   queda apuntando a las bases `_restore`;
2. dejar documentada de forma corta la receta operativa de restauracion de
   prueba y bootstrap validado;
3. revisar y decidir una ruta canonica unica para la configuracion local
...
1. leer esta seccion y `docs/diario_sesiones/2026-06-15.md`;
2. asumir ya validado:
   - el login real sobre la base buena con `superusuario`;
...
- resolver la ruta canonica de configuracion local en desarrollo:
  - hoy conviven `compartido` fuera de `src` y `src/compartido`;
  - la app en desarrollo lee `src/compartido/.env`;
  - esta dualidad ya ha provocado editar el `.env` equivocado durante una
    validacion manual y debe simplificarse o, como minimo, documentarse mejor.
...
- La configuracion local del repo sigue siendo confusa por la coexistencia de
  `compartido` fuera de `src` y `src/compartido`, con riesgo real de actuar
  sobre el `.env` equivocado.
